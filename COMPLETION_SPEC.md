...
- [~] Idle: the menu bar process wakes ≤ 1×/min with seconds off. **Measured 2026-09-18 with `RMAC_TOP_BAR_RENDER_COUNT_DIR`: the dev top-bar rendered 10 times in 70 s at idle — a real idle-wake bug.** The clock tick itself is correct (`delay_until_next_clock_tick` waits to the next minute when seconds are off), and the runtime already gates on change (`retain` sets `visible: next.status != previous.status` in `crates/rmac-shell-runtime/src/runtime.rs`), so a `status` field really is changing every ~7 s at idle. **Follow-up measurement 2026-09-18 disproved the focus-blip theory:** `niri msg --json focused-window` polled once a second for 30 s returned `null` **all 30 times** — there is no focused window at idle and it is stable, so `focused` is not the changing field. The redraw source is therefore another `rmac_shell_status::Snapshot` field that changes ~every 7 s without a visible difference (battery/sound are stable u8s; the prime suspects are the network indicator's Wi-Fi signal strength/scan or a PipeWire/UPower property). Next step: log the `Snapshot` diff at idle (or temporarily hash each indicator) to catch the churning field, then exclude it from the change gate. `RMAC_TOP_BAR_RENDER_COUNT_DIR` re-measures.
