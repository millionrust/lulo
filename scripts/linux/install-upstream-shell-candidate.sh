...
components=(wallpaper top-bar dock osd app-switcher screenshot mission-control)
...
echo "  components: wallpaper, menu bar, Dock, system OSD, app switcher, screenshots, Mission Control, and five first-party apps"
...
if [[ "$build" == true ]]; then
  (
    ...
    CARGO_TARGET_DIR="$target_dir" cargo "${shell_build[@]}" \
      ...
      --bin screenshot --bin mission-control
  )
...
if pgrep -f "^${target_dir}/${shell_profile}/(wallpaper|top-bar|dock|osd|app-switcher|screenshot|mission-control)( --service)?$" >/dev/null 2>&1; then
...
staged+=("$manifest_temporary")
{
  ...
  echo "components=rmac-wallpaper,rmac-top-bar,rmac-dock,rmac-osd,rmac-app-switcher,rmac-screenshot,rmac-mission-control,rmac-files,rmac-terminal,rmac-text-editor,rmac-system-monitor,rmac-system-settings"
} >"$manifest_temporary"
...
systemctl --user reset-failed \
  rmac-wallpaper.service rmac-top-bar.service rmac-dock.service rmac-osd.service \
    rmac-app-switcher.service rmac-screenshot.service rmac-mission-control.service >/dev/null 2>&1 || true

if systemctl --user is-active --quiet rmac-session.target; then
  systemctl --user restart \
    rmac-wallpaper.service rmac-top-bar.service rmac-dock.service rmac-osd.service \
    rmac-app-switcher.service rmac-screenshot.service rmac-mission-control.service
...
echo "Installed the pinned upstream shell, system OSD, app switcher, screenshots, Mission Control, and five first-party app candidates."
