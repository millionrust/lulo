...
| Appearance | Local pane choices remain, but shared read-only host snapshots and a writable, recoverable rmac theme authority are available | Settings portal plus `rmac-theme` | Replace local controls and apply scheme/accent/contrast/motion to every rmac surface |
