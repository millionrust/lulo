...
| Battery | Real battery/AC state, health, time/rate, physical capacity/cycles/model, and advertised power profiles; coalesced UPower plus modern/legacy profile signals; owner-loss/restart recovery; generation-safe authoritative resampling with a retained post-mutation refresh | UPower and power-profiles-daemon | Supported charge thresholds, bounded authoritative history, and Linux interaction/scale/accessibility evidence |
