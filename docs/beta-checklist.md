...
|---|---|---|---|
| 1 | Journey 5 Save-panel behavior passes in CI with `rmac-file-chooser` present. The earlier local failure may reflect an absent private portal backend; the runner now fails preflight when the helper is missing. Re-run the focused scenario with fresh binaries. The chooser is still not deployed in the reference live session. | agent | M |
| 2 | Accessibility remains a release gate: AT-SPI `EditableText` is absent upstream; ACC-08 and role/name/action gaps remain; Control-F2 is implemented but not live-confirmed; and no owner-run Orca/I3 audit exists. See journeys 1–5 and 8–9 below. | agent / owner / upstream | L |
