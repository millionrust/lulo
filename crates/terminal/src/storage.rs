...
mod tests {
    ...
    #[test]
    fn atomic_save_replaces_profile_without_leaving_a_temp_file() {
        let root = temp_root();
        std::fs::create_dir(&root).unwrap();
        let target = root.join("profile.txt");
        std::fs::write(&target, "Basic").unwrap();

        save(&RealStorage, &target, "Ocean").unwrap();

        assert_eq!(std::fs::read_to_string(&target).unwrap(), "Ocean");
        assert_eq!(std::fs::read_dir(&root).unwrap().count(), 1);
        std::fs::remove_dir_all(root).unwrap();
    }

    #[test]
    fn missing_profile_is_a_normal_first_launch() {
        ...
        assert_eq!(
            load_optional(&storage, Path::new("profile.txt")).unwrap(),
            None
        );
    }
    ...
    fn injected_load_directory_and_write_failures_remain_distinct() {
        ...
        let load_failure = load_optional(&load_storage, Path::new("profile.txt")).unwrap_err();
        let directory_failure =
            save(&directory_storage, Path::new("config/profile.txt"), "Basic").unwrap_err();
        let write_failure =
            save(&write_storage, Path::new("config/profile.txt"), "Basic").unwrap_err();
