...
impl Session {
    ...
    /// Host loss makes an in-flight compositor operation uncertain. Fail only
    /// that exact command and issue nothing until a replacement host is ready.
...
mod tests {
    ...
    #[test]
    fn host_loss_stops_commands_until_replacement_readiness() {
        let mut session = ready_session();
        let pending = session.apply(&plan(&["A", "B"], false)).command.unwrap();
