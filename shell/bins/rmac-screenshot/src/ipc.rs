//! Bounded local transport from the niri ⇧⌘3/4/5 binds to the resident
//! screenshot service.
//!
//! niri spawns `rmac-screenshot selection` (etc.) for each shortcut. That
//! process sends one fixed word to a user-private datagram socket and exits,
//! so the resident service opens its overlay within a frame instead of
//! starting a GPUI app per press, and keeps the floating thumbnail alive.
...
const SOCKET_NAME: &str = "screenshot.sock";
const MAX_WIRE_BYTES: usize = 32;

fn invalid(message: &'static str) -> io::Error {
    io::Error::new(io::ErrorKind::PermissionDenied, message)
}

/// `$XDG_RUNTIME_DIR/rmac/screenshot.sock`, refusing symlinks and
...
impl Listener {
    pub fn bind() -> io::Result<Self> {
        ...
        match fs::symlink_metadata(&path) {
            ...
            Ok(_) => return Err(invalid("the screenshot socket path is not a socket")),
            Err(error) if error.kind() == io::ErrorKind::NotFound => {}
            Err(error) => return Err(error),
        }
...
mod tests {
    ...
    fn decodes_only_the_exact_words() {
        assert_eq!(decode(b"screen"), Some(Command::Screen));
        assert_eq!(
            decode(b"selection-to-clipboard"),
            Some(Command::SelectionToClipboard)
        );
        assert_eq!(decode(b"toolbar"), Some(Command::Toolbar));
        assert_eq!(decode(b"screen\n"), None);
