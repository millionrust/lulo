...
- [~] **0.5 Make the shell consume `rmac-design`.** (`crates/rmac-shell-ui/src/tokens.rs` added: GPUI conversion, global live tokens, `install_appearance_watch` mirroring the app runtime, 18 semantic accessors; `shell_visuals` deleted; all 38 `visuals::*` uses + 3 text literals migrated in the four bins) — `87d5b2b`, `20e5e29`, `fd2c243`.
  ...
  **Caveats:** (a) remaining component-specific `rgba(0x…)`/`px(…)` literals in the bins are deferred to the Phase 1/3/4 restyles that define their tokens — the 1.9 gate will enforce them. (b) The screenshot pair is blocked: the reference display stopped presenting during the check, so `niri msg action screenshot-screen` only produced one frame (16:50); evidence is the unit test + render counters instead. (c) The dev installer writes `~/.local/libexec/rmac`, but the active user units run `/usr/libexec/rmac` (system install); the live test used a temporary, now-removed user drop-in.
...
- [ ] **1.6 Sidebar, table/list, toolbar, traffic lights (5.8, 5.13, 5.14, 5.16).**
- [ ] **1.7 Popover, sheet, alert, tooltip, progress, spinner, empty state (5.10–5.12, 5.15).** Remove `Toast` from normal flows (grep usages; replace with inline status or alert).
- [ ] **1.8 Icon set.** Create every symbol in §4.10 as original SVG; rework the 7 app icons to the squircle grid with light/dark/tinted variants; add the third-party plate rule to `rmac-icon`.
- [ ] **1.9 Token gate script.** New `scripts/check-design-tokens.sh`: fails if product UI source (`crates/{finder,system-settings,notes,terminal,text-editor,activity-monitor,app-drawer,launcher-app,quick-settings-app,notification-center-app}/src`, `shell/bins`, `shell/crates`) contains `rgb(0x`, `rgba(0x`, `hsla(`, or `.rounded(px(<literal>))`, except files named `*tests*`. Add to CI next to `check-shared-controls.sh`.
...
- [ ] **2.2 Extend the compositor action model.**
...
- [ ] **2.3 Window chrome rules in `shell.kdl`.**
  Do: active shadow `elev.window.active`, inactive shadow `elev.window.inactive` (niri `shadow` supports `inactive-color`); corner radius 12 default, 16 for app-ids with unified toolbars (`org.rmac.Files`, `org.rmac.SystemSettings`, `org.rmac.Notes`); remove `open-maximized true` for Files (macOS opens Finder at last size, default 920×560 centered); every rmac app opens floating, centered on the focused output at its last saved size (`rmac-window-state` already stores it).
  Third-party windows: floating, centered, 70% of output working area on first launch, remember per app-id.
  Verify: open each app → centered, correct radius; focus change swaps shadows; no niri focus ring/border anywhere.

- [ ] **2.4 Traffic-light actions wired.** In `rmac-ui::traffic_lights()`: close → `window.remove_window()`; minimize → `MinimizeWindow` via the compositor client; zoom → `FullscreenWindow` toggle; ⌥-click → `FillWindow`; 1 s hover on zoom → tiling menu (§5.16) sending `TileWindow`/`CenterWindow`/`MoveWindowToOutput`. Double-click toolbar background → Settings "Double-click a window's title bar to" (Zoom = Fill toggle, Minimize, Do Nothing).
...
- [ ] **3.3 App menus from `org.rmac.AppMenu1`.** Render exported menus with the §5.9 menu. The **app menu** (bold name) for every first-party app must contain, in order: About {App} · ─ · Settings… ⌘, (if the app has settings) · ─ · Services › (disabled, until implemented) · ─ · Hide {App} ⌘H · Hide Others ⌥⌘H · Show All · ─ · Quit {App} ⌘Q. Hide/Hide Others/Show All call the Phase 2 actions. Raise `MAX_ITEMS_PER_MENU` from 32 → 64 and add submenu support to the wire format (`WireItem` gets an optional `children` vector; bump interface to `org.rmac.AppMenu2`, keep v1 reader for one release).
...
- [ ] **3.5 Status items (right side), right-to-left order:** clock · Control Center · Search · Focus · Now Playing (When Active) · Sound · Bluetooth · Battery · Wi-Fi/Ethernet/VPN · third-party StatusNotifierItems · privacy indicator.
...
- [ ] **4.8 Context menu (right-click or press-and-hold 500 ms).** Rows by state:
  ```
...
- [ ] **4.11 Minimize target.** When minimize into app icon is off, minimized windows appear as live thumbnail tiles (static snapshot taken at minimize, scaled to tile, with app badge 20 px bottom-right) in the right group; click restores.
- [ ] **4.12 Autohide.** Hidden: shelf slides off-edge 250 ms; reveal when pointer touches the edge for 0 ms (Tahoe default delay), hide 500 ms after leaving. Menu open prevents hiding. Setting in Desktop & Dock: "Automatically hide and show the Dock". ⌥⌘D toggles autohide.
