...
| Lock Screen | Real live lock and capability-gated automatic-suspend choices; truthful Hidden preview state; secure manual/logind/pre-sleep and idle paths | `org.rmac.LockScreen1`, bounded action-free notification lock projection, logind `CanSuspend`/`Suspend(false)`, niri `ext-session-lock-v1`, supervised PAM-enabled swaylock, validated swayidle policy, and delay inhibitor | PAM-capable rmac presentation provider, preview controls, scoped build, and Linux suspend/security/accessibility evidence |
