...
| NOTES-09 | P1 | M | Fixed <sha> (Find in note) | Mac separates Note List Search ⌥⌘F from in-note Find ⌘F, plus Find and Replace ⇧⌘F. / Lulo's ⌘F only focuses the list search — no in-note find or replace. ⌘F now opens a case-insensitive in-note Find bar (⌘G/⇧⌘G cycle matches); Note List Search moved to ⌥⌘F. Find and Replace (⇧⌘F) is still absent. | `crates/notes/src/note_find_controller.rs`, `NOTES_MENUS` |
| NOTES-10 | P1 | S | Fixed <sha> (verified) | Mac's File menu has Export as › PDF. / Lulo has it implemented but missing from the live menu (as TE-04); the PDF save panel also starts in the process's working directory, not Documents. Verified in source: `notes::ExportNotePdf` is registered, in `NOTES_MENUS`, and covered by `rmac-notes`'s test suite; the save panel's starting directory is unchanged (still not Documents). | `print_controller.rs:102-155`, `:114` |
