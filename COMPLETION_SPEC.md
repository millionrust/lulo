...
- [~] Idle: the menu bar process wakes ≤ 1×/min with seconds off. **Measured 2026-09-18 with `RMAC_TOP_BAR_RENDER_COUNT_DIR`: the dev top-bar rendered 10 times in 70 s at idle — a real idle-wake bug.** The clock tick itself is correct (`delay_until_next_clock_tick` waits to the next minute when seconds are off), and the runtime already gates on change (`retain` sets `visible: next.status != previous.status` in `crates/rmac-shell-runtime/src/runtime.rs`), so a `status` field really is changing every ~7 s at idle. The battery/sound indicators are all stable (u8 percentage/volume, state, flags), and `clock` is settings — so the change is almost certainly the **focus projection blip** already documented in 3.3 (the projected focused app id flips between `None` and `Some(…)` at idle), which changes `rmac_shell_status::Snapshot::focused` and forces a redraw. The fix is the root-cause one from 3.3: stop the focus projection from emitting a transient `None` when focus has not really changed (or have the runtime's change gate ignore it), then re-measure with `RMAC_TOP_BAR_RENDER_COUNT_DIR`.
