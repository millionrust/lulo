...
mod tests {
    ...
    fn embedded_icons_are_bounded_self_contained_vectors() {
        ...
        for icon in icons {
            let svg = icon.svg();
            // Generated by scripts/build-icons.py on the 1024 icon grid.
            assert!(svg.contains("Original rmac artwork, MIT licensed."));
            assert!(
                svg.contains("<svg xmlns=\"http://www.w3.org/2000/svg\" viewBox=\"0 0 1024 1024\"")
            );
