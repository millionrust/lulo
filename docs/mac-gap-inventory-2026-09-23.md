...
| Desktop widgets | A | Tahoe desktop widgets (Clock, Weather, Calendar, Reminders, Photos). They depend on apps #6–#9. |
| Desktop icons / Stacks | p | `crates/rmac-desktop` (356 lines), rendered by `shell/bins/rmac-wallpaper`. Missing **Use Stacks** (group by kind), clean-up/sort, and the Get Info and rename menu. |
