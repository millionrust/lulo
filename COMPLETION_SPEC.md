...
- [~] **2.2 Extend the compositor action model.** (First slice against real niri 26.04 IPC, which exposes only *focused-window* actions and **no native minimize/hide**: added `FullscreenWindow{window,on}`, `FillWindow{window}`, `CenterWindow{window}` to `rmac-compositor::Action`/`ActionKind`, matching `wire::Action` variants, capabilities, and a focus-then-act `convert_action_sequence` that `execute_at` now sends in order. `cargo test -p rmac-compositor -p rmac-compositor-niri` ✔ 9+8 with new wire-format tests. **Remaining:** `TileWindow`, `MoveWindow`, `ResizeWindow` (niri has no absolute placement; approximate via `set-column-width`/move), and `Minimize/Restore/Hide/Unhide/ShowDesktop` via the `rmac-parking` workspace design below.)
