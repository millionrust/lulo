...
- [~] **3.5 Status items (right side), right-to-left order:** clock · Control Center · Search · Focus · Now Playing (When Active) · Sound · Bluetooth · Battery · Wi-Fi/Ethernet/VPN · third-party StatusNotifierItems · privacy indicator. (**Open bug found 2026-09-17 on the reference PC:** after the session was locked and unlocked, the trailing status indicators stopped rendering and only the clock shows. It reproduces with both the packaged `/usr/libexec` bar and the dev build; NetworkManager/BlueZ/UPower are reachable on the system bus (`Connectivity=4`), `rmac-focus`/`rmac-notification-center` are active, no unit failed, and `rmac-top-bar` logs no error, so `rmac_shell_runtime::watch` is running without a published service snapshot. Needs a probe of `read_service_batch`/`rmac_network::network_snapshot()` in the shell context. Separately fixed a real 0.2 regression: the promoted bin packages resolved `assets/...` under their own dir instead of `shell/assets`, so the rmac mark and OSD icons silently failed to load — corrected to `../../assets/...` (`634d073`), mark verified rendering.)
