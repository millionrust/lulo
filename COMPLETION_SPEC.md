...
- [~] **1.8 Icon set.** Create every symbol in §4.10 as original SVG; rework the 7 app icons to the squircle grid with light/dark/tinted variants; add the third-party plate rule to `rmac-icon`. (Added the `rmac-icon` third-party plate rule (`IconShape`, `PLATE_ICON_SCALE = 0.8`, `third_party_plate`, tested — `cargo test -p rmac-icon` ✔ 12) and a starter set of 18 original 16×16/1.5 px line symbols under `shell/assets/symbols/` (search, chevrons, checkmark, xmark, plus, minus, ellipsis, gear, list, grid, sidebar-left, folder, trash-empty, lock, bell); all SVGs parse cleanly. **Remaining:** the rest of the §4.10 symbol list (~40: battery variants, wifi-0..3/off/error, ethernet, vpn, bluetooth-connected, speaker-0..3/muted, mic, camera, screen-record, focus, airplane, display, now-playing, play/pause/next/prev, power/restart/sleep/logout, share, tag, info, columns, gallery, brightness/keyboard-brightness) and the 7 app-icon squircle rework with light/dark/tinted variants.) — `c20b09a`
