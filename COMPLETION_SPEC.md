...
- [~] **4.9 Drag and drop.** (Investigated: the renderer-neutral drag model is complete in `crates/rmac-dock/src/drag.rs` (`DragSession::begin/update/finish`, `ReorderIntent::revalidate`, `DropOutcome`), but the Dock host consumes `SurfaceDescription` and has no press/move/release wiring, and `DragSession::begin` needs a `ShelfLayoutPlan` from `presentation::prepare_layout` that the runtime does not yet expose in its snapshot. So this pass needs: expose the layout plan to the host, add drag session state + `on_mouse_move`/`on_mouse_up` to the tiles, render the `preview_order` parting, and commit `UpdatePins(MoveTo)` on drop. Verified only that a synthetic drag currently does not reorder.) Drag a kept app horizontally → neighbors part with 200 ms spring; drop to reorder (persist). Drag an app tile up ≥ 1.5 tiles out of the Dock and release → "Remove" puff (tooltip "Remove" shown while outside) for kept, non-running apps; running apps unpin but stay in recent group. Drag an app from Apps/Files into the app group → insert and pin. Drag files onto an app tile → highlight tile if app's MimeType accepts, open with it on drop. Drag files onto Trash → move to Trash via `rmac-places-system` with undo available in Files. Drag a folder to the right group → add folder stack. Drag a mounted volume onto Trash → tile changes to eject symbol and ejects.
