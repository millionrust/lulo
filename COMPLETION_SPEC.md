...
- [~] **4.8 Context menu (right-click or press-and-hold 500 ms).** (Verified live: right-clicking the Dock's Firefox tile opened a dark menu — *Firefox* · Open · New Window · New Private Window · Open Profile Manager · ─ · Remove from Dock · Show in Finder — with a hover tooltip under the tile. Still to do: window-title rows with ✓/•, the Options `›` submenu (Keep in Dock/Open at Login/Show in Files), Hide/Quit with ⌥→Force Quit, and the Trash/folder menus.) Rows by state:
  ```
