...
| Lock Screen | Placeholder; secure manual/logind/pre-sleep lock plus validated idle-timeout authority exist, but no settings are exposed | niri `ext-session-lock-v1`, supervised PAM-enabled swaylock, validated swayidle policy, logind signals/hint, and delay inhibitor | Live Settings API, supported suspend choices, preview authority, scoped build, and Linux suspend/security/accessibility evidence |
