...
| Sound | Real devices, defaults, volume, and mute; local-only alert/startup/UI-sound controls removed; coalesced PipeWire graph monitoring with outage/recovery state; generation-safe full resampling and retained post-mutation refresh | PipeWire/WirePlumber through bounded argument-separated `wpctl` and `pw-mon` | Machine-readable/native inventory, routes, profiles, per-channel balance, and Linux interaction/scale/accessibility evidence |
