...
//! 18 pt frame at x 81, the 13 pt bold title starts at x 100 and " — Edited"
//! follows in the tertiary colour. The title's ▾ opens the document menu,
//! which in rmac holds the encoding and line-ending choices the saver backs.
...
impl EditorView {
    pub(super) fn render_title_bar(&self, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        let row = div()
            ...
            .child(
                Button::new("document-actions", "")
                    .icon(Icon::new(IconName::ChevronDown).text_color(mac::text_secondary()))
                    .ghost()
                    .xsmall()
                    .disabled(format_locked)
                    .tooltip(format_status.clone())
