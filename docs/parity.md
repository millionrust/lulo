...
| OTHER-02 | P1 | M | Missing | Mac: Save, Export, Print and Page Setup slide down as sheets attached under the document's title bar (the document's close button greys out); Open is the app's own window. / Lulo: every chooser is a separate floating dialog window (`org.rmac.FileChooser`), centred on screen and not attached — seen over another app's window. | `crates/rmac-file-chooser/src/main.rs:100` |
| OTHER-03 | P1 | S | Broken | Mac: the 880×448 Open panel's buttons sit 20 pt above the bottom edge. / Lulo (live, installed build): the Open panel's Cancel/Open row is cut off by the window's bottom edge, and the compact Save panel's Save button is clipped at the right edge. | `crates/rmac-file-chooser/src/panel_view.rs:676`, `metrics.rs` |
| OTHER-04 | P1 | S | Broken | Mac: while an Open/Save panel is up, the menu bar still belongs to the app (TextEdit). / Lulo (live): the bar switches to "FileChooser" with no menus. | `rmac-file-chooser` app id, `shell/bins/rmac-menubar` |
| OTHER-05 | P2 | S | Missing | Mac: the Open window is titled "Open" and the Save sheet has no title; the panel opens on the document's folder or the last one used. / Lulo: windows titled "Open File" / "Save File", and Open started in Downloads. | `crates/rmac-file-chooser/src/main.rs:80` |
| OTHER-06 | P1 | M | Missing | Mac sidebar (121 pt): Recents, Shared, Favourites (Applications, Downloads, Documents, Desktop, added folders), Locations (iCloud Drive, home, disks, network), Tags (7 colours + All Tags…), Media (Music, Photos, Movies). / Lulo: Recents, Shared, Favourites (Downloads, Documents, Desktop), Locations (home) — no Applications, Tags or Media; the sidebar is so narrow names truncate ("Downlo…", "Docum…"). | `crates/rmac-file-chooser/src/panel.rs:149` |
| OTHER-07 | P2 | M | Missing | Mac: the panel's view button offers Icons ⌘1, List ⌘2, Columns ⌘3 (no gallery) and Hide Sidebar, next to a Group/Sort button; the expanded Save browser opens in Columns. / Lulo: Icons and List only. | `crates/rmac-file-chooser/src/browser.rs` |
| OTHER-08 | P2 | S | Missing | Mac Open panel bottom row: the app's accessory (TextEdit "New Document"), Show Options/Hide Options (app options such as encoding), Cancel, Open (greyed until a selection). / Lulo: Cancel and Open only. | `crates/rmac-file-chooser/src/panel_view.rs:1332` |
| OTHER-09 | P1 | S | Missing | Mac collapsed Save sheet, 390×218: Save As, Tags, File Format popup (plus Plain Text Encoding for text), Where popup + disclosure, Cancel · Save; the name field selects only the base name ("test"); expanded (880×448) adds the sidebar/browser and New Folder at bottom-left. Export as PDF uses a 422×182 variant with Show Details. / Lulo: 390×145 with Save As, Where and an expand chevron — no Tags; File Format only when the app passes more than one filter; "test.txt" is selected whole, extension included. | `crates/rmac-file-chooser/src/panel.rs:318`, `panel_view.rs:575` |
| OTHER-10 | P2 | S | Missing | Mac: New Folder in the Save panel opens a 320×158 sheet "New Folder" / "Name of new folder inside “lulo-audit”:" / field "untitled folder" / Cancel · Create. / Lulo: silently creates "untitled folder" and opens it. | `crates/rmac-file-chooser/src/panel.rs:970` |
| OTHER-11 | P2 | S | Partial | Mac replace alert (a 260 pt sheet on the Save sheet): "“test.rtf” already exists. Do you want to replace it?" / "A file or folder with the same name already exists in the folder “lulo-audit”. Replacing it will overwrite its current contents.", Cancel · Replace (112×30). / Lulo: the first sentence only; Cancel is drawn with the default accent fill and Replace plain, yet Return saves. | `crates/rmac-file-chooser/src/panel_view.rs:1212` |
