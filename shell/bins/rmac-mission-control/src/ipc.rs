//! Bounded local transport from niri binds, the Dock and hot corners to the
//! resident Mission Control service.
//!
//! niri spawns `rmac-mission-control mission-control` for ⌃↑ (and the other
//! commands). That process sends one fixed word to a user-private datagram
//! socket and exits, so the resident service (which already holds the window
//! list and icons) answers within a frame instead of starting a GPUI app per
//! press.
...
/// `$XDG_RUNTIME_DIR/rmac/mission-control.sock`, refusing symlinks and
...
impl Listener {
    pub fn bind() -> io::Result<Self> {
        ...
        match fs::symlink_metadata(&path) {
            ...
            Ok(_) => return Err(invalid("the Mission Control socket path is not a socket")),
            Err(error) if error.kind() == io::ErrorKind::NotFound => {}
            Err(error) => return Err(error),
        }
...
mod tests {
    ...
    fn decodes_only_the_exact_words() {
        assert_eq!(decode(b"mission-control"), Some(Command::MissionControl));
        assert_eq!(decode(b"previous-space"), Some(Command::PreviousSpace));
        assert_eq!(decode(b"cancel"), Some(Command::Cancel));
        assert_eq!(decode(b"show-desktop\n"), None);
