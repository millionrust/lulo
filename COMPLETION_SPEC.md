...
- [~] **3.3 App menus from `org.rmac.AppMenu1`.** Render exported menus with the §5.9 menu. The **app menu** (bold name) for every first-party app must contain, in order: About {App} · ─ · Settings… ⌘, (if the app has settings) · ─ · Services › (disabled, until implemented) · ─ · Hide {App} ⌘H · Hide Others ⌥⌘H · Show All · ─ · Quit {App} ⌘Q. Hide/Hide Others/Show All call the Phase 2 actions. Raise `MAX_ITEMS_PER_MENU` from 32 → 64 and add submenu support to the wire format (`WireItem` gets an optional `children` vector; bump interface to `org.rmac.AppMenu2`, keep v1 reader for one release). (**Verified live:** the promoted Files binary owns `org.rmac.Files.Menu` and `Menus` returns the real File/Edit menus (busctl-confirmed); the bar renders `Finder · File · Edit · View`. **Blocking bug found and root-caused:** the projected focused app id flips between `None` and `Some("org.rmac.Files")` even at idle, so the bar clears the menus and closes an open menu whenever it goes `None` (which is why clicking a title appeared to do nothing). Locally mitigated by retrying the menu fetch (`ae3ef63`), but the fix is to make the projected focus stable — investigate the compositor watcher's focus projection. Still to do: the bold-name app menu, `MAX_ITEMS_PER_MENU` 32→64, and `AppMenu2` submenus.)
