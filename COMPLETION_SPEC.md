...
- Global shortcuts actually registered: only `Mod+Space` (launcher) and `Mod+Ctrl+Q` (lock), plus niri binds in `shell.kdl` (Ctrl+←/→ spaces, Ctrl+↑ overview, Mod+Ctrl+F fullscreen, media keys → `rmac-osd`, Mod+Tab / Mod+Shift+Tab → `rmac-app-switcher`, Mod+Shift+3/4/5 (+Ctrl) and Print → `rmac-screenshot`, Mod+grave MRU).
...
| `elev.window.active` | niri: `softness 32 spread 0 offset 0,12 color #00000073` | design-lab |
| `elev.window.inactive` | niri: `softness 12 spread 0 offset 0,4 color #0000004D` | design-lab |
...
- [~] **2.3 Window chrome rules in `shell.kdl`.** (`packaging/rmac-session/shell.kdl`: the unscoped application-window rule gives every first- and third-party app the standardized 16 px curve, clipping, no focus ring/border, and the design-lab active/inactive shadows (active `0 12 32 #00000073`, inactive `0 4 12 #0000004D`); Files opens at its last saved size and keeps its source-list blur. `niri validate` and live captures for all six first-party app IDs pass on the reference PC. **Remaining:** third-party first-launch 70%-of-working-area sizing and per-app-id remember (niri has no such rule — needs the shell).)
...
- [~] **2.7 Spaces and Mission Control.** (**Done 2026-09-23, ADR 0014, `shell/bins/rmac-mission-control`, measured in `design-lab/mission-control.html`:** ⌃↑ rmac Mission Control over the xray wallpaper with window pictures cut from one grim read, the 72/164 pt Spaces bar with Desktop pills, 138 × 90 thumbnails, + (names niri's spare workspace) and ⊗ (moves windows back, unnames), hover ring and title capsule, click to focus; ⌃↓ App Exposé with captions; F11 Show Desktop via the spare empty workspace; ⌃←/⌃→ skip `rmac-parking`; hot corners in Desktop & Dock. **Remaining:** live thumbnails (PipeWire window casts), ⌃1…⌃9, F3, drag between Spaces, keeping the Dock visible, three-finger Space swipes (niri-owned gestures).) (Measured earlier: the Spaces strip is a single **"Desktop" pill** at top centre with a **+** at the far right, not a filmstrip of numbered Spaces; windows spread without overlapping, each labelled with its app name, and the Dock stays visible.) Map: ⌃↑ = niri overview (exists) presented as Mission Control; ⌃↓ = App Exposé (overview filtered to the focused app — if niri cannot filter, open the app switcher window list instead, see 5.7); ⌃←/⌃→ switch Space (exists); ⌃1…⌃9 go to Space N; F3/Mission Control key = overview. Spaces bar: niri overview shows workspaces; ensure `rmac-parking` never appears (name-filter via niri `workspace` `open-on-output` + hiding rule; if niri shows it anyway, park windows off-screen in a floating position instead and document).
...
- [x] Idle: the menu bar process wakes ≤ 1×/min with seconds off. **Fixed and verified 2026-09-18.** The wake source was `NetworkIndicator.wifi_strength` (raw Wi-Fi percent) drifting 49↔52 every few seconds; `rmac-shell-status::NetworkIndicator::wifi_bars()` now buckets it into the menu-bar's **three** bars (0–32 / 33–65 / 66–100, matching §3.10's "3 of 3 bars") and `rmac-shell-runtime::publication` computes `visible` from that bar-normalized status, so sub-bar drift no longer requests a frame (health-only publications are preserved). Measured with `RMAC_TOP_BAR_RENDER_COUNT_DIR`: **2 renders in 120 s** (1/min, the clock tick), down from 10 in 70 s. The temporary diagnostic (`20bc4c6`, `ccb4efe`) was reverted (`c7f7ea6`, `cbc049a`). **Measured 2026-09-18 with `RMAC_TOP_BAR_RENDER_COUNT_DIR`: the dev top-bar rendered 10 times in 70 s at idle — a real idle-wake bug.** The clock tick itself is correct (`delay_until_next_clock_tick` waits to the next minute when seconds are off), and the runtime already gates on change (`retain` sets `visible: next.status != previous.status` in `crates/rmac-shell-runtime/src/runtime.rs`), so a `status` field really is changing every ~7 s at idle. **Root cause found 2026-09-18 by instrumenting `publication`:** the focus-blip theory was disproved (`niri msg focused-window` was `null` for 30/30 idle samples, stable), and a temporary per-field log in `rmac-shell-runtime::publication` showed the churning field is **`network` — 12 changes in 75 s**, versus `sound`/`outputs`/`focus`/`bluetooth`/`battery` at ~1 each. `NetworkIndicator.wifi_strength` stores the raw NetworkManager signal percent (`crates/rmac-shell-status/src/state.rs`), which drifts a point or two every few seconds; it is consumed **only for the accessible label** ("signal N percent") and the bars glyph, never as drawn text, so the change is not consumer-visible. **Fix:** expose Wi-Fi signal as bars (the spec §3.10 example is "3 of 3 bars") and quantize the stored value to those buckets so sub-bar drift no longer changes `Snapshot`; then re-measure with `RMAC_TOP_BAR_RENDER_COUNT_DIR`. The temporary instrumentation was committed (`20bc4c6`) and reverted (`c7f7ea6`). **Final form (`3825390`):** the volatile percent field was removed entirely — `NetworkIndicator` now carries `wifi_bars: Option<u8>` computed at the source (`state.rs`), and the runtime's `visible_status` normalisation was deleted, so sub-bar Wi-Fi drift never changes the snapshot at all. `rmac-shell-status`/`rmac-top-bar`/`rmac-shell-runtime`/`rmac-top-bar-runtime` tests ✔ (9/9/7/17) and `rmac-shell-ui` ✔ 12.
...
- [~] **Screenshots** — bin `rmac-screenshot` (ADR 0010, measured in `design-lab/screenshot.html`). Done: ⇧⌘3 / ⌃⇧⌘3 screen; ⇧⌘4 / ⌃⇧⌘4 selection with the x/y then W×H readout (Space toggles window mode with the rgba(146,184,228,.5) highlight; Esc cancels); ⇧⌘5 toolbar (close, Entire Screen / Window / Selection, Options, Capture; dashed selection with 8 handles, 50 % dim outside, move/resize/redraw); Options: Save to Desktop|Documents|Downloads|Clipboard, Timer None/5/10, Show Floating Thumbnail, Remember Last Selection, Show Mouse Pointer (persisted in `~/.config/rmac/screenshot.conf`); capture through `grim` (niri wlr-screencopy), clipboard through `wl-copy`; shutter cue; floating thumbnail 155 wide, 12 from the right and 102 from the bottom, slides in, holds 5 s, slides out, then saves; click saves and opens the file, swipe right saves now; files named `Screenshot 2026-09-23 at 1.36.39 PM.png` (U+202F before PM). Control Center's Screenshot control opens the toolbar. Still to do: recording (Record Entire Screen / Record Selection via the ScreenCast portal + PipeWire, menu-bar stop button + privacy dot), Mail/Preview/Other Location…, dragging the thumbnail into apps, Markup on click, a timer countdown, multi-output selection, window captures without overlapping windows.
...
- [ ] **6.3 Greeter branding (GDM).** Keep GDM as the login authority. Provide only the gschema override: no logo (the Mac login window has none), 12-hour clock with weekday, blue accent, Inter, background `rmac-aurora.svg`. Do not replace GDM for 1.0. GDM cannot draw the Mac composition (`design-lab/lock.html` §4): no large date/clock over the user list, no blurred-wallpaper backdrop in greeter mode (GNOME Shell paints the theme's `#lockDialogGroup` colour; `picture-uri` only reaches GNOME sessions), no bottom Sleep/Restart/Shut Down row (power lives in the top-bar menu), no pill-shaped "Enter Password" field or shake, and its avatar, name and prompt styles come from the distro's shell theme gresource, which rmac does not patch.
...
- [~] **8.6 Performance harness.** (FEEL_SPEC.md §D.12 idle-silence audit done: every `Duration::from_secs(1)` in the shell/runtime crates is a reconnect/retry backoff after an authority failure (`rmac-dock-runtime`, `rmac-audio`, `rmac-bluetooth`, portal, focus, gtk-settings, locale), not an idle poll; the menu-bar clock is minute-aligned and there is no per-second redraw timer. The latency/idle **budgets still need measurement** on release builds. **Idle-CPU probe 2026-09-18 (debug builds):** `wallpaper` 2.27%, `dock` 2.27%, but **`top-bar` ≈35%** sustained — a real busy loop in the menu bar, not rendering (its render counter is flat at 1/min after the `wifi_bars` fix). Since the Dock uses the same GPUI/debug stack at 2.27%, the spin is specific to the top-bar code. **Per-thread probe:** the CPU is in the `blocking` crate's pool — `(blocking-1)` and `(blocking-2)` at ~8.9% each — with the main thread only 2.2% and `async-io` 1.4%, so something calls `blocking::unblock` repeatedly at idle (the menubar's own calls — shortcut dispatch, recent items, `spawn_command` — are event-driven, as are `rmac-shell-runtime`'s settings/notifications and `rmac-top-bar-runtime`'s `hour_cycle`, so the caller is still to be found). **Root cause pinned 2026-09-18 (gdb + refresh log):** gdb caught the busy blocking thread in `blocking::unblock … <rmac_shell_runtime::coordinator::ServiceBatch, rmac_shell_runtime::runtime::consume::{closure_env#3}>` = `read_service_batch` at `runtime.rs:247`. Logging each `Refresh` showed **`Refresh({audio: true})` fires ~4–5×/s at idle** (`network` only ~2/min). The audio watcher (`crates/rmac-audio/src/linux.rs`) reads `pw-mon` stdout, coalesces for `WATCH_QUIET_PERIOD` 75 ms / `WATCH_MAX_COALESCE` 250 ms, and publishes `WatchEvent::Changed` on every flush — so a continuously chatty `pw-mon` yields ~4 Changed/s, each triggering a **blocking `wpctl get-volume` subprocess read** on the bridge's blocking pool (two threads ~9% each). **Partial fix (`1c5c7d0`):** the `Refresh({audio})` storm comes from a **second** `pw-mon` watcher inside `rmac-shell-status-linux` (not `rmac-audio`'s); its 75 ms `QUIET_PERIOD` flushed several times a second. Widening it to **500 ms** cut idle CPU **35% → 17%**, and to **1 s** (`1856a4d`) to **~12%** (measured). The two `rmac-audio` constant tweaks that did not move CPU were reverted (`15e7da4`, `57b3708`). **Remaining:** each flush still rebuilds the full audio snapshot (`rmac_audio::snapshot()` → `pw-dump` subprocess + JSON parse) via `read_service_batch`; measured, `pw-dump` costs **~20–30 ms and 177 KB** per call (plus parse), which at 1 flush/s accounts for the remaining cost — measured **release-build** idle CPU is **5.0%** (vs 11.8% debug), still ~50× the ≤0.1% budget and therefore a real, build-independent violation; make the bridge read only the cheap volume/mute level (or keep the snapshot and diff), and consider consolidating the two PipeWire watchers. **Why `pw-mon` cannot be the source (measured):** it prints a line for every graph event and this reference graph emits **~1100 events/s at idle even with the top-bar stopped** (5506 lines/5 s), so the watcher is reading a torrent and `Refresh` fires continuously; `pactl` is not installed, so `pactl subscribe` (which reports only sink/source events) is unavailable. The correct fix is a **native PipeWire client** that subscribes to the registry and reacts only to the default node / volume / mute changes (no subprocess, no line parsing), keeping the full `rmac_audio::Snapshot` for Quick Settings only when visible. This is the real, still-open task; the flush-rate bound (`1c5c7d0`, `1856a4d`) is mitigation only. A cheaper `wpctl` read alone does not reach ≤0.1% because any per-second subprocess is ~0.2–0.5% CPU in release. (Temp refresh log `09e47f5` reverted.) Extend `scripts/measure-baseline.py` to Linux with: session-ready time (GDM auth → all shell units READY), Spotlight shortcut→first frame, Control Center click→first frame, menu open latency, Dock hover→tooltip frame, app cold/warm launch, idle CPU and wakeups per shell process over 10 min (`pidstat -w`), RSS after 8 h. Budgets (from `PLAN_V2.md` §6.4 plus these): session ready ≤ 3 s; overlay open ≤ 80–100 ms p95; interaction ≤ 50 ms p95; 60 Hz animations ≥ 99% frames ≤ 16.67 ms; each shell process idle CPU ≤ 0.1%, wakeups ≤ 1/s; total shell RSS ≤ 350 MB (S, revise only with evidence).
...
| ⌃↑ / F3 | Mission Control | `rmac-mission-control` (ADR 0014; F3 not bound yet) |
| ⌃↓ | App Exposé | `rmac-mission-control` |
| ⌃← / ⌃→ | Previous/next Space | `rmac-mission-control` (skips parking) |
| ⌃1…⌃9 | Go to Space N | niri |
| F4 | Apps | `rmac-apps` |
| F11 (fn F11) | Show Desktop | `rmac-mission-control` (shows the spare empty Space) |
