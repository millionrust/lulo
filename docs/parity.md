...
| FILES-38 | P1 | S | Missing | Mac: File ▸ Delete Immediately… (⌥⌘⌫) works on any selection in any folder and opens an app-modal alert window, 260×250: caution icon with a Finder badge, "Are you sure you want to delete “test.txt”?" / "This item will be deleted immediately. You can’t undo this action.", Cancel · Delete (112×30 each; Delete red, neither blue). / Lulo: the command only works inside the Bin view; its in-window card reads "Delete Item Permanently?" and makes Delete the focused default. | `crates/finder/src/view/permanent_delete_controller.rs:4`, `dialog_presentation/recovery.rs:136` |
...
| TE-18 | P1 | M | Missing | Mac: closing an edited saved document just autosaves; closing an edited Untitled one slides the Save sheet down (458×367) headed "Do you want to keep this new document “Untitled”?" / "You can choose to save your changes or delete this document immediately…", with Save As, Tags, Where, Plain Text Encoding and Delete (bottom-left) … Cancel · Save; Esc cancels. / Lulo: any dirty document gets a 260 pt in-window card "Do you want to save the changes you made?" / "Your changes will be lost if you don't save them." Cancel · Don't Save · Save, and Esc and ⌘. do nothing. | `crates/text-editor/src/view/render/alert.rs:43`, `view/alerts.rs:6` |
...
| PREV-15 | P1 | S (menu) | Missing | Mac File: New from Clipboard ⌘N, Open Recent ▸, Close All, Save ⌘S, Save As… ⌥⇧⌘S, Duplicate ⇧⌘S, Rename…, Move To…, Revert To ▸, Enter Password…, Edit Permissions… (339×487 sheet), Import from Camera/Scanner, Take Screenshot ▸, Export… (381×419 sheet: Export As, Tags, Where, Format, Quartz Filter, PDF/A and image options, Permissions…), Export as PDF…, Share ▸, Print…. / Lulo's menu is Open…, Close Window, Print… only — PREV-04 is marked Fixed but no Save or Export command reaches the menu. | `PREVIEW_MENUS` |
...
| MON-12 | P1 | S | Broken | Mac: Quit Process ⌥⌘Q is greyed until a process is selected. / Lulo (live): Quit Process ⌘⌫ and Force Quit Process… are enabled with nothing selected and do nothing when chosen. | `crates/activity-monitor/src/view.rs:166` |
...
| CLOCK-08 | P1 | S | Broken | Mac: an empty Alarms tab shows only "No Alarms". / Lulo (live): a giant grey alarm-clock glyph fills the window behind "No Alarms". | `crates/clock/src/view.rs` |
