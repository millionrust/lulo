...
| Language & Region | Real locale1 assignments and installed-locale inventory; validated LANG mutation preserving every LC_* override; exact preview; one-step rollback; property-change/restart stream with reconnect; authoritative refresh; explicit sign-out requirement; read-only X11/console keyboard metadata | `rmac-locale`, `rmac-locale-linux`, `org.freedesktop.locale1`, `locale -a` | niri input-source/layout editing, formatting preview, and Linux polkit/restart/scale/accessibility evidence |
...
Keyboard pane and is not falsely changed through localed. Format previews and
