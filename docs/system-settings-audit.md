...
| Sound | Real machine-readable device identity, friendly labels, verified defaults, volume, mute, advertised ports, and hardware profiles; exact node/device/active-profile/route associations and availability gate mutations; private authority names are redacted and never persisted; missing or ambiguous capability data has a separate failure state; local-only alert/startup/UI-sound controls removed; coalesced PipeWire graph monitoring with outage/recovery state; generation-safe full resampling and retained post-mutation refresh | PipeWire/WirePlumber through bounded argument-separated `wpctl`, `pw-dump`, and `pw-mon` | Capability-gated per-channel balance and Linux hotplug/restart/suspend/interaction/scale/accessibility evidence |
