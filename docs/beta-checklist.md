...
| 3 | Journey 6 (System Monitor): the process table's total AT-SPI blackout is fixed; re-run this pass live against a fresh build reconfirms the same remaining gap as before — its AT-SPI projection is bounded to the visible viewport (19 of ~227 processes this run) with no accessible way (search `EditableText`, column-header `Action`) to bring a specific low-usage row into it, so the accept flow (select, Quit, confirm, cancel, confirm again, process ends) still hasn't been exercised. Lifting the viewport bound needs a shared `rmac_ui`/`gpui-component` `Table` change (cross-cutting, out of proportion for this pass). | agent | S–M |
...
| 6 | Inspect resource use, safely stop a process | `scripts/linux/run-journey-monitor.py` | **Re-run 2026-09-25 (evening)** live against a fresh `rmac-system-monitor` build (`6f38352e`), its own disposable `sleep` process, one instance, temp desktop entry removed afterward: 2/6 steps pass (launch, disposable-process-started). The script deliberately still stops short of Quit/Force Quit — its own safety rule (never invoke them without independently confirming the disposable process is selected over AT-SPI) held. Two of the three blocking gaps are unchanged from the last pass and out of proportion to fix here: the search field still has no AT-SPI `EditableText` (the same pinned `accesskit_unix` upstream gap as Spotlight/Terminal/Text Editor, not fixable in this repo) and the process table's AT-SPI projection is still bounded to its current visible viewport (this run: 19 of ~227 real processes) rather than the full ≤300-row model `accessibility.rs`/`process_table.rs` already build — column headers still have no AT-SPI `Action` to sort by PID either, so nothing can bring an idle disposable process into that window without typing (blocked by the same `EditableText` gap) or a rewrite of the shared `rmac_ui`/`gpui-component` `Table`'s virtualization to also mount off-screen rows for accessibility, a cross-cutting change out of scope for this pass (same "Blocked, not Gap" shape as `PopUpButton`/`ContextMenu`/`ListRow` in `docs/accessibility-audit.md`). New this pass: `quit_controls_exist` (the top bar's Process menu / "Force Quit Process…") was not found over AT-SPI this run either — `docs/journey-suite.md` already tracks this as the same category-menu AT-SPI flakiness journey 5 has, live session load was high (8.44) during this run, and the live top bar was not rebuilt from this branch (the brief forbids restarting the owner's live services), so it is unrelated to this pass's ⌃F2 changes. | **Fail** — reconfirmed, not regressed; the accept flow (select, Quit, confirm, cancel, confirm again, process ends) still needs the process table's AT-SPI viewport limit lifted before it can be exercised by AT-SPI alone |
