...
- [~] Idle: the menu bar process wakes ≤ 1×/min with seconds off. **Measured 2026-09-18 with `RMAC_TOP_BAR_RENDER_COUNT_DIR`: the dev top-bar rendered 10 times in 70 s at idle — a real idle-wake bug.** The clock tick itself is correct (`delay_until_next_clock_tick` waits to the next minute when seconds are off), and the runtime already gates on change (`retain` sets `visible: next.status != previous.status` in `crates/rmac-shell-runtime/src/runtime.rs`), so a `status` field really is changing every ~7 s at idle. The likely culprit is a volatile field the bar does not display (e.g. the battery time estimate from UPower, or a PipeWire/NetworkManager property) landing in the compared `rmac_shell_status::Snapshot`; the fix is to exclude non-displayed volatile fields from the projection (or gate on the displayed fields only), then re-measure with `RMAC_TOP_BAR_RENDER_COUNT_DIR`.
