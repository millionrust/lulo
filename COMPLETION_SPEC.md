...
- [~] **1.9 Token gate script.** New `scripts/check-design-tokens.sh`: fails if product UI source (`crates/{finder,system-settings,notes,terminal,text-editor,activity-monitor,app-drawer,launcher-app,quick-settings-app,notification-center-app}/src`, `shell/bins`, `shell/crates`) contains `rgb(0x`, `rgba(0x`, `hsla(`, or `.rounded(px(<literal>))`, except files named `*tests*`. Add to CI next to `check-shared-controls.sh`. (Script created with a `--report` mode and an allowlist for the token-conversion modules (`rmac-design`, `shell/crates/rmac-shell-ui/src/tokens.rs`). **`shell/bins` is now clean** — all 44 color and 19 radius literals replaced with `rmac-shell-ui::tokens` lookups, with shell build/clippy/lib-tests green. **Remaining:** 131 literals across the app crates (activity-monitor 20, finder 27, notes 26, system-settings 27, terminal 19, app-drawer 5, launcher-app 5, text-editor 1), so the gate is not yet wired to CI to avoid a red build; each needs a semantic token mapping.) — `5f4791d`, `f335f48`, `9069001`, `ac79c38`
