...
| Notifications | Live service-backed app/policy list with desktop-entry names/icons plus allow/block, top-bar badge, and history controls with separate stream/mutation errors; paired with an on-demand translucent Notification Center panel using the same authoritative grouped history, clear/read mutations, and per-app disable policy | `org.rmac.NotificationCenter1`, `rmac-notification-center-panel`, plus live XDG app catalog | Banner/sound UI, lock-preview controls after secure preview presentation is integrated, history actions, and Linux interaction/accessibility evidence |
