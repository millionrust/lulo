...

{
  if command -v systemctl >/dev/null 2>&1; then
    for unit in rmac-session.target rmac-safe-mode.target \
      ...
      rmac-launcher.service rmac-quick-settings.service rmac-notification-center.service rmac-notification-center-panel.service rmac-focus.service rmac-wallpaper.service \
