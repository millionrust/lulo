...
streams. Focus has its own timedate/logind hints and runtime authority;
Notification Center unread/urgent state now arrives through its private typed
D-Bus signal. The D2 top-bar UI and real niri/hardware behavior remain unproven
until those streams are exercised on the Linux reference PC.
