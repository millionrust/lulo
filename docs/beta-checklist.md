...
| 2 | Journey 5 (Text Editor via the portal): the nested `save-untitled` scenario passes in CI when `rmac-file-chooser` is built. The earlier local failure has not been tied to an app defect; the private test portal has no FileChooser backend when that helper is absent, and `run_lulo.py` previously allowed that configuration to proceed. It now fails preflight unless the helper is supplied. Re-run the scenario with the helper included after the current package build. The live reference session still has no deployed `rmac-file-chooser.service`. | coordinator | M |
