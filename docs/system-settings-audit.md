...
| Sound | Real machine-readable device identity, friendly labels, verified defaults, volume, and mute; private authority names are redacted and never persisted; local-only alert/startup/UI-sound controls removed; coalesced PipeWire graph monitoring with outage/recovery state; generation-safe full resampling and retained post-mutation refresh | PipeWire/WirePlumber through bounded argument-separated `wpctl`, `pw-dump`, and `pw-mon` | Routes, profiles, per-channel balance, and Linux interaction/scale/accessibility evidence |
