...
pub fn render_niri_fallback(
    ...
    let mut output = String::from(
        "// Generated by Lulo OS. Include this file only when the GlobalShortcuts portal is unavailable.\n\n",
    );
...
pub fn write_niri_fallback(path: &Path, dispatcher: &Path) -> Result<(), Error> {
    ...
    write_generated_niri_bindings(path, contents.as_bytes())
}

/// Atomically select exactly one Linux shortcut backend. The package-owned
/// niri config includes this generated file, so an active portal leaves it
/// empty while a portal failure installs only the reviewed fallback bindings.
pub fn write_niri_binding_mode(
    path: &Path,
    dispatcher: &Path,
    ...
    let contents = match status {
        BackendStatus::Portal { .. } => {
            "// Generated by Lulo OS. GlobalShortcuts portal owns system shortcuts.\n".to_owned()
        }
        BackendStatus::FallbackRequired { .. } => {
            render_niri_fallback(&default_shortcuts(), dispatcher)?
        }
    };
    write_generated_niri_bindings(path, contents.as_bytes())
}

fn write_generated_niri_bindings(path: &Path, contents: &[u8]) -> Result<(), Error> {
    let parent = path
        .parent()
    ...
    rmac_storage::atomic_write_private(path, contents)
        .map_err(|error| Error::new(Operation::WriteFallback, error.to_string()))
}
